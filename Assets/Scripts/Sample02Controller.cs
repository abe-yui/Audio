...
public class Sample02Controller : MonoBehaviour
{
	[SerializeField]
	...
	[SerializeField]
	private AudioMixerSnapshot seGrasslandSnapshot;
	[SerializeField]
	private AudioMixerSnapshot seCaveSnapshot;
	[SerializeField]
	private AudioMixerSnapshot voiceGrasslandSnapshot;
	[SerializeField]
	private AudioMixerSnapshot voiceCaveSnapshot;
	...
	public void ChangedChorusToggle()
	{
	...
	public void ChangedFieldToggle()
	{
		if (fieldToggle.isOn)
		{
			seCaveSnapshot.TransitionTo(fieldTransitionTime);
			voiceCaveSnapshot.TransitionTo(fieldTransitionTime);
		}
		else
		{
			seGrasslandSnapshot.TransitionTo(fieldTransitionTime);
			voiceGrasslandSnapshot.TransitionTo(fieldTransitionTime);
		}
	}
	...
	public void ChangedMenuToggle()
	{
		...
		else
		{
			...
			openMenu.Play();
			defaultSnapshot.TransitionTo(menuTransitionTime);
		}
	}
	...
	{
		StartCoroutine(StartChorusCoroutine());
	}

	IEnumerator StartChorusCoroutine()
	{
		// エンカウントSE
		encounter.Play();

		// BGM
		yield return DoCrossFadeBGM(Chorus, field, 2);
	...
	public void FinishChorus()
	{
		StartCoroutine(FinishChorusCoroutine());
	}

	IEnumerator FinishChorusCoroutine()
	{
		// WinSE
		resultWin.Play();

		// BGM
		yield return DoFadeOutBGM(Chorus, 1);
